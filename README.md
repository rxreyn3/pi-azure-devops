...
Published package name: `@rxreyn3/pi-azure-devops`.

Add the scope registry mapping in the consuming project or user npm config:

```ini
@rxreyn3:registry=https://npm.pkg.github.com
```

Authenticate outside this repo. For local CLI use with GitHub Packages, use a classic GitHub PAT with `read:packages` for installs and `write:packages` for publishing. Store token-bearing npm config only in user-level or environment-specific configuration; never commit token values.

Install in OMP or another consumer project:
...
1. Confirm the mock CLI command exits successfully and uses no network/token.
...
Manual publication and live-install steps are documented in `docs/publication.md`. Remote queue/cancel/rerun mutation remains not implemented.
